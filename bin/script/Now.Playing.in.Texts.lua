...
function meta_changed()
    ...
    vlc.msg.info("Playback stopped")
	update_files(true)
  elseif vlc.playlist.status() == "playing" then
    vlc.msg.info("Playback resumed")
	update_files(false)
  else
	update_files(true)
  end
...
names = { "filename", "filename1", "title", "artist", "album",
    ...
    "now_playing", "playing_state", "time_at_recent_pause",
...
---------------
-- update files
function update_files(stopped)
  print_debug("update_files()")
  metadata = get_metadata(stopped)
  write_all_files()
end
...
-- get metadata (safe)
function get_metadata(stopped)
  
  local rslt = {}
  rslt[is_playing] = vlc.input.is_playing()
  
  local state = playing_states.paused
  if stopped == true then
		vlc.msg.info("Playback stopped")
	else
		state = playing_states.playing
  end
  ...
  local current_time_us = vlc.var.get(vlc.object.input(), "time")
  vlc.msg.dbg("Raw current_time_us value: " .. current_time_us)
   
  local current_time_ms = current_time_us / 1000
  local current_time = current_time_ms / 1000
  local current_minutes = math.floor(current_time / 60)
  local current_seconds = math.floor(current_time % 60)
  local current_time_formatted = string.format("%d:%02d", current_minutes, current_seconds)
  
  vlc.msg.dbg("Formatted current time: " .. current_time_formatted)
  
  rslt["time_at_recent_pause"] = current_time_formatted
  
  rslt["playing_state"] = state
